#!/usr/bin/env python3
"""Generate hhodo_cuts.param from timing_window_setup output."""
import argparse
import re
...
PATTERNS = {
    'pos_min': re.compile(r'^hhodo_(1x|1y|2x|2y)_PosAdcTimeWindowMin=\s*(.*)$'),
    'pos_max': re.compile(r'^hhodo_(1x|1y|2x|2y)_PosAdcTimeWindowMax=\s*(.*)$'),
    'neg_min': re.compile(r'^hhodo_(1x|1y|2x|2y)_NegAdcTimeWindowMin=\s*(.*)$'),
    'neg_max': re.compile(r'^hhodo_(1x|1y|2x|2y)_NegAdcTimeWindowMax=\s*(.*)$'),
}
...
def main():
    ...
    ap.add_argument('--template', default='PARAM/HMS/HODO/hhodo_cuts.param',
    ...
    with open(args.template) as f:
        for line in f:
            if line.startswith('hhodo_PosAdcTimeWindowMin'):
                break
    ...
    with open(args.output, 'w') as out:
        for h in header_lines:
            out.write(h + '\n')
        out.write('\n')
        out.write(format_array('hhodo_PosAdcTimeWindowMin', arrays['PosAdcTimeWindowMin']))
        out.write('\n\n')
        out.write(format_array('hhodo_PosAdcTimeWindowMax', arrays['PosAdcTimeWindowMax']))
        out.write('\n\n')
        out.write(format_array('hhodo_NegAdcTimeWindowMin', arrays['NegAdcTimeWindowMin']))
        out.write('\n\n')
        out.write(format_array('hhodo_NegAdcTimeWindowMax', arrays['NegAdcTimeWindowMax']))
        out.write('\n')
